shadow$provide.module$node_modules$vega_scale$build$vega_scale = function(require, module, exports) {
  (function(global, factory) {
    typeof exports === "object" && typeof module !== "undefined" ? factory(exports, require("module$node_modules$vega_util$build$vega_util"), require("module$node_modules$vega_time$build$vega_time")) : typeof define === "function" && define.amd ? define(["exports", "vega-util", "vega-time"], factory) : (global = typeof globalThis !== "undefined" ? globalThis : global || self, factory(global.vega = {}, global.vega, global.vega));
  })(this, function(exports, vegaUtil, vegaTime) {
    function bandSpace(count, paddingInner, paddingOuter) {
      paddingInner = count - paddingInner + paddingOuter * 2;
      return count ? paddingInner > 0 ? paddingInner : 1 : 0;
    }
    function invertRange(scale) {
      return function(_) {
        ...
        let t;
        _ < lo && (t = lo, lo = _, _ = t);
        return [scale.invert(lo), scale.invert(_)];
      };
    }
    function invertRangeExtent(scale) {
      return function(_) {
        ...
        t = 0;
        for (n = range.length; t < n; ++t) {
          range[t] >= lo && range[t] <= _ && (min < 0 && (min = t), max = t);
        }
        if (!(min < 0)) {
          return lo = scale.invertExtent(range[min]), _ = scale.invertExtent(range[max]), [lo[0] === void 0 ? lo[1] : lo[0], _[1] === void 0 ? _[0] : _[1]];
        }
      };
    }
    ...
    function bisector(f) {
      ...
      f.length !== 2 ? (compare1 = ascending$1, compare2 = (d, x) => ascending$1(f(d), x), delta = (d, x) => f(d) - x) : (compare1 = f === ascending$1 || f === descending$1 ? f : zero$1, delta = compare2 = f);
    ...
    function cubehelix$2(h, s, l$jscomp$0, opacity) {
      if (arguments.length === 1) {
        ...
        if (JSCompiler_temp instanceof Cubehelix) {
          ...
          JSCompiler_temp instanceof Rgb || (JSCompiler_temp = rgbConvert(JSCompiler_temp));
          var g = JSCompiler_temp.g / 255, b = JSCompiler_temp.b / 255, l = (BC_DA * b + JSCompiler_temp.r / 255 * -1.7884503806 - 3.5172982438 * g) / (BC_DA + -1.7884503806 - 3.5172982438);
          b -= l;
          var k = (1.97294 * (g - l) - -0.29227 * b) / -0.90649;
          b = (g = Math.sqrt(k * k + b * b) / (1.97294 * l * (1 - l))) ? Math.atan2(k, b) * degrees$1 - 120 : NaN;
          JSCompiler_temp = new Cubehelix(b < 0 ? b + 360 : b, g, l, JSCompiler_temp.opacity);
        }
      } else {
        JSCompiler_temp = new Cubehelix(h, s, l$jscomp$0, opacity == null ? 1 : opacity);
      }
    ...
    function basisClosed(values) {
      ...
      return function(t) {
        ...
        return basis((t - i / n) * n, values[(i + n - 1) % n], values[i % n], values[(i + 1) % n], values[(i + 2) % n]);
      };
    }
    ...
    function object(a, b) {
      ...
      for (k in b) {
        k in a ? i[k] = interpolate$1(a[k], b[k]) : c[k] = b[k];
      }
      return function(t) {
        for (k in i) {
          c[k] = i[k](t);
    ...
    function string(a, b) {
      ...
      for (b += ""; (am = reA.exec(a)) && (bm = reB.exec(b));) {
        (bs = bm.index) > bi && (bs = b.slice(bi, bs), s[i] ? s[i] += bs : s[++i] = bs), (am = am[0]) === (bm = bm[0]) ? s[i] ? s[i] += bm : s[++i] = bm : (s[++i] = null, q.push({i, x:interpolateNumber(am, bm)})), bi = reB.lastIndex;
      }
    ...
    function interpolateTransform(parse, pxComma, pxParen, degParen) {
      ...
      function translate(xa, ya, xb, yb, s, q) {
        xa !== xb || ya !== yb ? (s = s.push("translate(", null, pxComma, null, pxParen), q.push({i:s - 4, x:interpolateNumber(xa, xb)}, {i:s - 2, x:interpolateNumber(ya, yb)})) : (xb || yb) && s.push("translate(" + xb + pxComma + yb + pxParen);
      }
      function rotate(a, b, s, q) {
        a !== b ? (a - b > 180 ? b += 360 : b - a > 180 && (a += 360), q.push({i:s.push(pop(s) + "rotate(", null, degParen) - 2, x:interpolateNumber(a, b)})) : b && s.push(pop(s) + "rotate(" + b + degParen);
      }
      function skewX(a, b, s, q) {
        a !== b ? q.push({i:s.push(pop(s) + "skewX(", null, degParen) - 2, x:interpolateNumber(a, b)}) : b && s.push(pop(s) + "skewX(" + b + degParen);
      }
      function scale(xa, ya, xb, yb, s, q) {
        xa !== xb || ya !== yb ? (s = s.push(pop(s) + "scale(", null, ",", null, ")"), q.push({i:s - 4, x:interpolateNumber(xa, xb)}, {i:s - 2, x:interpolateNumber(ya, yb)})) : xb === 1 && yb === 1 || s.push(pop(s) + "scale(" + xb + "," + yb + ")");
      }
      return function(a, b) {
        var s = [], q = [];
        a = parse(a);
    ...
    function piecewise(interpolate, values) {
      values === void 0 && (values = interpolate, interpolate = interpolate$1);
    ...
    function transformer$2() {
      function rescale() {
        ...
        clamp !== identity$2 && (clamp = clamper(domain[0], domain[n - 1]));
      ...
      var domain = unit, range = unit, interpolate = interpolate$1, transform, untransform, unknown, clamp = identity$2, piecewise, output, input;
      scale.invert = function(y) {
        return clamp(untransform((input ||= piecewise(range, domain.map(transform), interpolateNumber))(y)));
      };
      scale.domain = function(_) {
      ...
      scale.clamp = function(_) {
        return arguments.length ? (clamp = _ ? !0 : identity$2, rescale()) : clamp !== identity$2;
      };
    ...
    function formatLocale$1(locale) {
      ...
      var group = locale.grouping === void 0 || locale.thousands === void 0 ? identity$1 : formatGroup(map$1.call(locale.grouping, Number), locale.thousands + ""), currencyPrefix = locale.currency === void 0 ? "" : locale.currency[0] + "", currencySuffix = locale.currency === void 0 ? "" : locale.currency[1] + "", decimal = locale.decimal === void 0 ? "." : locale.decimal + "", numerals = locale.numerals === void 0 ? identity$1 : formatNumerals(map$1.call(locale.numerals, String)), percent = locale.percent === 
    ...
    function tickFormat$1(start, stop, count, specifier) {
      count = tickStep(start, stop, count);
      specifier = formatSpecifier(specifier == null ? ",f" : specifier);
      switch(specifier.type) {
        case "s":
          start = Math.max(Math.abs(start), Math.abs(stop));
          if (stop = specifier.precision == null) {
            var precision = isNaN;
            count = Math.max(0, Math.max(-8, Math.min(8, Math.floor(exponent(start) / 3))) * 3 - exponent(Math.abs(count)));
            stop = !precision(precision = count);
          }
          stop && (specifier.precision = precision);
          return formatPrefix(specifier, start);
        case "":
        case "e":
        case "g":
        case "p":
        case "r":
          var JSCompiler_temp;
          if (JSCompiler_temp = specifier.precision == null) {
            precision = isNaN, start = Math.max(Math.abs(start), Math.abs(stop)), count = Math.abs(count), start = Math.abs(start) - count, count = Math.max(0, exponent(start) - exponent(count)) + 1, JSCompiler_temp = !precision(precision = count);
          }
          JSCompiler_temp && (specifier.precision = precision - (specifier.type === "e"));
          break;
        case "f":
        case "%":
          specifier.precision != null || isNaN(precision = Math.max(0, -exponent(Math.abs(count)))) || (specifier.precision = precision - (specifier.type === "%") * 2);
      }
    ...
    function linear() {
      var scale = transformer$2()(identity$2, identity$2);
      scale.copy = function() {
    ...
    function loggish(transform) {
      ...
      scale.tickFormat = (count, specifier) => {
        ...
        typeof specifier !== "function" && (base % 1 || (specifier = formatSpecifier(specifier)).precision != null || (specifier.trim = !0), specifier = format$1(specifier));
        if (count === Infinity) {
          return specifier;
        }
      ...
      scale.nice = () => domain(nice(domain(), {floor:x => pows(Math.floor(logs(x))), ceil:x => pows(Math.ceil(logs(x)))}));
      return scale;
    }
    function log() {
    ...
    function powish(transform) {
      ...
      scale.exponent = function(_) {
        return arguments.length ? (exponent = +_, exponent === 1 ? transform(identity$2, identity$2) : exponent === 0.5 ? transform(transformSqrt, transformSquare) : transform(transformPow(exponent), transformPow(1 / exponent))) : exponent;
      };
      return linearish(scale);
    }
    function pow() {
    ...
    function ticker(year, month, week, day, hour, minute) {
      function tickInterval(start, stop, count) {
        const target = Math.abs(stop - start) / count, i = bisector(([, , step]) => step).right(tickIntervals, target);
      ...
      const tickIntervals = [[second, 1, 1000], [second, 5, 5E3], [second, 15, 15E3], [second, 30, 3E4], [minute, 1, 6E4], [minute, 5, 3E5], [minute, 15, 9E5], [minute, 30, 18E5], [hour, 1, 36E5], [hour, 3, 108E5], [hour, 6, 216E5], [hour, 12, 432E5], [day, 1, 864E5], [day, 2, 1728E5], [week, 1, 6048E5], [month, 1, 2592E6], [month, 3, 7776E6], [year, 1, 31536E6]];
    ...
    function calendar(ticks, tickInterval, year, month, week, day, hour, minute, second, format) {
      ...
      var scale = transformer$2()(identity$2, identity$2), invert = scale.invert, domain = scale.domain, formatMillisecond = format(".%L"), formatSecond = format(":%S"), formatMinute = format("%I:%M"), formatHour = format("%I %p"), formatDay = format("%a %d"), formatWeek = format("%b %d"), formatMonth = format("%B"), formatYear = format("%Y");
      scale.invert = function(y) {
    ...
    function transformer$1() {
      ...
      var x0 = 0, x1 = 1, t0, t1, k10, transform, interpolator = identity$2, clamp = !1, unknown;
      scale.domain = function(_) {
    ...
    function transformer() {
      ...
      var x0 = 0, x1 = 0.5, x2 = 1, s = 1, t0, t1, t2, k10, k21, interpolator = identity$2, transform, clamp = !1, unknown;
      scale.domain = function(_) {
    ...
    function band() {
      function rescale() {
        var n = domain().length;
        const reverse = range$1[1] < range$1[0], stop = range$1[1 - reverse], space = bandSpace(n, paddingInner, paddingOuter);
        ...
        n = range(n).map(i => start + step * i);
        return ordinalRange(reverse ? n.reverse() : n);
      }
      const scale = ordinal().unknown(void 0), domain = scale.domain, ordinalRange = scale.range;
      let range$1 = [0, 1], step, bandwidth, round = !1, paddingInner = 0, paddingOuter = 0, align = 0.5;
      ...
      scale.invertRange = function(_) {
        if (_[0] != null && _[1] != null) {
          var reverse = range$1[1] < range$1[0], values = reverse ? ordinalRange().reverse() : ordinalRange(), n = values.length - 1, lo = +_[0], hi = +_[1];
          if (lo === lo && hi === hi) {
            if (hi < lo) {
              var t = lo;
              lo = hi;
            ...
            if (!(hi < values[0] || lo > range$1[1 - reverse])) {
              return _ = Math.max(0, bisectRight(values, lo) - 1), hi = lo === hi ? _ : bisectRight(values, hi) - 1, lo - values[_] > bandwidth + 1e-10 && ++_, reverse && (t = _, _ = n - hi, hi = n - t), _ > hi ? void 0 : domain().slice(_, hi + 1);
            }
          }
        }
      };
      scale.invert = function(_) {
        return (_ = scale.invertRange([_, _])) ? _[0] : _;
      };
      scale.copy = function() {
    ...
    function scaleBinOrdinal() {
      function scale(x) {
        return x == null || x !== x ? void 0 : range[(bisectRight(domain, x) - 1) % range.length];
      }
      let domain = [], range = [];
      scale.domain = function(_) {
        return arguments.length ? (domain = map.call(_, vegaUtil.toNumber), scale) : domain.slice();
      };
      scale.range = function(_) {
    ...
    function create(type, constructor, metadata) {
      const ctr = function() {
        const s = constructor();
        s.invertRange || (s.invertRange = s.invert ? invertRange(s) : s.invertExtent ? invertRangeExtent(s) : void 0);
        s.type = type;
    ...
    function scale(type, scale, metadata) {
      return arguments.length > 1 ? (scales.set(type, create(type, scale, metadata)), this) : isValidScaleType(type) ? scales.get(type) : void 0;
    }
    function isValidScaleType(type) {
      return scales.has(type);
    }
    function hasType(key, type) {
      return (key = scales.get(key)) && key.metadata[type];
    }
    ...
    function interpolate(type, gamma) {
      type = $[method(type)];
      return gamma != null && type && type.gamma ? type.gamma(gamma) : type;
    }
    ...
    function colors(palette) {
      if (vegaUtil.isArray(palette)) {
        return palette;
      }
      const n = palette.length / 6 | 0, c = Array(n);
      for (let i = 0; i < n;) {
    ...
    function scheme(name, scheme) {
      ...
      return arguments.length > 1 ? (schemes[name] = scheme, this) : schemes[name];
    }
    function validTicks(scale, ticks, count) {
      var range = scale.range();
      let lo = range[0], hi = vegaUtil.peek(range);
      range = ascending;
      lo > hi && (range = hi, hi = lo, lo = range, range = descending);
      lo = Math.floor(lo);
      hi = Math.ceil(hi);
      ticks = ticks.map(v => [v, scale(v)]).filter(_ => lo <= _[1] && _[1] <= hi).sort(range).map(_ => _[0]);
      if (count > 0 && ticks.length > 1) {
        for (range = [ticks[0], vegaUtil.peek(ticks)]; ticks.length > count && ticks.length >= 3;) {
          ticks = ticks.filter((_, i) => !(i % 2));
        }
        ticks.length < 3 && (ticks = range);
      }
      return ticks;
    }
    ...
    function tickFormat(locale, scale, count, specifier, formatType, noSkip) {
      ...
      if (type === "time" || formatType === "time") {
        ...
        if (noSkip || scale.bins) {
          ...
          const test = tickLog(scale, count, !1);
          format = _ => test(_) ? varfmt(_) : "";
        }
      } else {
        scale.tickFormat ? (scale = scale.domain(), format = locale.formatSpan(scale[0], scale[scale.length - 1], count, specifier)) : specifier && (format = locale.format(specifier));
      }
      return format;
    }
    function tickLog(scale, count, values) {
      ...
      scale = d => {
        d /= Math.pow(base, Math.round(Math.log(d) / logb));
        d * base < base - 0.5 && (d *= base);
        return d <= k;
      };
      return values ? ticks.filter(scale) : scale;
    }
    function labelValues(scale, count) {
      scale.bins ? (scale = scale.bins, count = scale.slice(0, -1), count.max = vegaUtil.peek(scale), scale = count) : scale.type === "log" ? scale = tickLog(scale, count, !0) : symbols[scale.type] ? (scale = scale[symbols[scale.type]](), scale = [-Infinity].concat(scale), scale.max = Infinity) : scale = tickValues(scale, count);
      return scale;
    }
    function labelFormat(locale, scale, count, type, specifier, formatType, noSkip) {
      if (formats[scale.type] && formatType !== "time" && formatType !== "utc") {
        count = scale[formats[scale.type]]();
        formatType = count.length;
        noSkip = formatType > 1 ? count[1] - count[0] : count[0];
        let i;
        for (i = 1; i < formatType; ++i) {
          noSkip = Math.min(noSkip, count[i] - count[i - 1]);
        }
        locale = locale.formatSpan(0, noSkip, 30, specifier);
      } else {
        locale = tickFormat(locale, scale, count, specifier, formatType, noSkip);
      }
      return type === "symbol" && (symbols[scale.type] || scale.bins) ? formatRange(locale) : type === "discrete" ? formatDiscrete(locale) : formatPoint(locale);
    }
    const bisectRight = bisector(ascending$1).right;
    ...
    const e10 = Math.sqrt(50), e5 = Math.sqrt(10), e2 = Math.sqrt(2), implicit = Symbol("implicit");
    ...
    define(Rgb, rgb$1, extend(Color, {brighter(k) {
    ...
    define(Hsl, hsl$2, extend(Color, {brighter(k) {
      k = k == null ? brighter : Math.pow(brighter, k);
    ...
    const radians = Math.PI / 180, degrees$1 = 180 / Math.PI, t0$1 = 4 / 29, t1$1 = 6 / 29, t2 = 3 * t1$1 * t1$1, t3 = t1$1 * t1$1 * t1$1;
    define(Lab, lab$1, extend(Color, {brighter(k) {
    ...
    define(Hcl, hcl$2, extend(Color, {brighter(k) {
    ...
    define(Cubehelix, cubehelix$2, extend(Color, {brighter(k) {
      k = k == null ? brighter : Math.pow(brighter, k);
      ...
      var h = isNaN(this.h) ? 0 : (this.h + 120) * radians, l = +this.l, a = isNaN(this.s) ? 0 : this.s * l * (1 - l), cosh = Math.cos(h);
      h = Math.sin(h);
      return new Rgb(255 * (l + a * (-0.14861 * cosh + 1.78277 * h)), 255 * (l + a * (-0.29227 * cosh + -0.90649 * h)), 255 * (l + 1.97294 * cosh * a), this.opacity);
    }}));
    var constant = x => () => x, rgb = function rgbGamma(y) {
      ...
      rgb.gamma = rgbGamma;
      return rgb;
    }(1), rgbBasis = rgbSpline(basis$1), rgbBasisClosed = rgbSpline(basisClosed), reA = /[-+]?(?:\d+\.?\d*|\.?\d+)(?:[eE][-+]?\d+)?/g, reB = new RegExp(reA.source, "g"), degrees = 180 / Math.PI, identity$3 = {translateX:0, translateY:0, rotate:0, skewX:0, scaleX:1, scaleY:1}, svgNode, interpolateTransformCss = interpolateTransform(function(value) {
      value = new (typeof DOMMatrix === "function" ? DOMMatrix : WebKitCSSMatrix)(value + "");
      return value.isIdentity ? identity$3 : decompose(value.a, value.b, value.c, value.d, value.e, value.f);
    ...
    }, ", ", ")", ")"), zoom = function zoomRho(rho, rho2, rho4) {
      function zoom(p0, p1) {
      ...
      return zoom;
    }(Math.SQRT2, 2, 4), hsl$1 = hsl(hue$1), hslLong = hsl(nogamma), hcl$1 = hcl(hue$1), hclLong = hcl(nogamma), cubehelix$1 = cubehelix(hue$1), cubehelixLong = cubehelix(nogamma), $ = Object.freeze({__proto__:null, interpolate:interpolate$1, interpolateArray:function(a, b) {
      return (isNumberArray(b) ? numberArray : genericArray)(a, b);
    }, interpolateBasis:basis$1, interpolateBasisClosed:basisClosed, interpolateCubehelix:cubehelix$1, interpolateCubehelixLong:cubehelixLong, interpolateDate:date$1, interpolateDiscrete:function(range) {
      var n = range.length;
      return function(t) {
        return range[Math.max(0, Math.min(n - 1, Math.floor(t * n)))];
      };
    }, interpolateHcl:hcl$1, interpolateHclLong:hclLong, interpolateHsl:hsl$1, interpolateHslLong:hslLong, interpolateHue:function(a, b) {
      var i = hue$1(+a, +b);
      return function(t) {
        t = i(t);
        return t - 360 * Math.floor(t / 360);
      };
    }, interpolateLab:function(start, end) {
    ...
    }, X:x => Math.round(x).toString(16).toUpperCase(), x:x => Math.round(x).toString(16)}, map$1 = Array.prototype.map, prefixes = "y z a f p n µ m  k M G T P E Z Y".split(" "), locale$1, format$1, formatPrefix;
    (function(definition) {
    ...
    const t0 = new Date(), t1 = new Date(), millisecond = timeInterval(() => {
    }, (date, step) => {
      date.setTime(+date + step);
    }, (start, end) => end - start);
    millisecond.every = k => {
      k = Math.floor(k);
      return isFinite(k) && k > 0 ? k > 1 ? timeInterval(date => {
    ...
    utcHour.range;
    const timeDay = timeInterval(date => date.setHours(0, 0, 0, 0), (date, step) => date.setDate(date.getDate() + step), (start, end) => (end - start - (end.getTimezoneOffset() - start.getTimezoneOffset()) * 6E4) / 864E5, date => date.getDate() - 1);
    timeDay.range;
    ...
    var pads = {"-":"", _:" ", 0:"0"}, numberRe = /^\s*\d+/, percentRe = /^%/, requoteRe = /[\\^$*+?|[\]().{}]/g, locale, timeFormat, utcFormat;
    (function(definition) {
    ...
    const slice = Array.prototype.slice, scales = new Map(), VEGA_SCALE = Symbol("vega_scale");
    ...
    scale("sequential-sqrt", function() {
    ...
    scale("bin-ordinal", scaleBinOrdinal, ["discrete", "discretizing"]);
    ...
    schemePastel2 = colors$1("b3e2cdfdcdaccbd5e8f4cae4e6f5c9fff2aef1e2cccccccc"), schemeSet1 = colors$1("e41a1c377eb84daf4a984ea3ff7f00ffff33a65628f781bf999999"), schemeSet2 = colors$1("66c2a5fc8d628da0cbe78ac3a6d854ffd92fe5c494b3b3b3"), schemeSet3 = colors$1("8dd3c7ffffb3bebadafb807280b1d3fdb462b3de69fccde5d9d9d9bc80bdccebc5ffed6f");
    const schemes = {};
    apply({accent:schemeAccent, category10:schemeCategory10, category20:"1f77b4aec7e8ff7f0effbb782ca02c98df8ad62728ff98969467bdc5b0d58c564bc49c94e377c2f7b6d27f7f7fc7c7c7bcbd22dbdb8d17becf9edae5", category20b:"393b795254a36b6ecf9c9ede6379398ca252b5cf6bcedb9c8c6d31bd9e39e7ba52e7cb94843c39ad494ad6616be7969c7b4173a55194ce6dbdde9ed6", category20c:"3182bd6baed69ecae1c6dbefe6550dfd8d3cfdae6bfdd0a231a35474c476a1d99bc7e9c0756bb19e9ac8bcbddcdadaeb636363969696bdbdbdd9d9d9", dark2:schemeDark2, observable10:schemeObservable10, 
    paired:schemePaired, pastel1:schemePastel1, pastel2:schemePastel2, set1:schemeSet1, set2:schemeSet2, set3:schemeSet3, tableau10:"4c78a8f58518e4575672b7b254a24beeca3bb279a2ff9da69d755dbab0ac", tableau20:"4c78a89ecae9f58518ffbf7954a24b88d27ab79a20f2cf5b43989483bcb6e45756ff9d9879706ebab0acd67195fcbfd2b279a2d6a5c99e765fd8b5a5"}, colors);
    apply({blues:"cfe1f2bed8eca8cee58fc1de74b2d75ba3cf4592c63181bd206fb2125ca40a4a90", greens:"d3eecdc0e6baabdda594d3917bc77d60ba6c46ab5e329a512089430e7735036429", greys:"e2e2e2d4d4d4c4c4c4b1b1b19d9d9d8888887575756262624d4d4d3535351e1e1e", oranges:"fdd8b3fdc998fdb87bfda55efc9244f87f2cf06b18e4580bd14904b93d029f3303", purples:"e2e1efd4d4e8c4c5e0b4b3d6a3a0cc928ec3827cb97566ae684ea25c3696501f8c", reds:"fdc9b4fcb49afc9e80fc8767fa7051f6573fec3f2fdc2a25c81b1db21218970b13", blueGreen:"d5efedc1e8e0a7ddd18bd2be70c6a958ba9144ad77319c5d2089460e7736036429", 
    ...
    purpleOrange:"4114696647968f83b7b9b4d6dadbebf3eeeafce0bafbbf74e8932fc5690d994a07", redBlue:"8c0d25bf363adf745ef4ae91fbdbc9f2efeed2e5ef9dcae15da2cb2f78b3134b85", redGrey:"8c0d25bf363adf745ef4ae91fcdccbfaf4f1e2e2e2c0c0c0969696646464343434", yellowGreenBlue:"eff9bddbf1b4bde5b594d5b969c5be45b4c22c9ec02182b82163aa23479c1c3185", redYellowBlue:"a50026d4322cf16e43fcac64fedd90faf8c1dcf1ecabd6e875abd04a74b4313695", redYellowGreen:"a50026d4322cf16e43fcac63fedd8df9f7aed7ee8ea4d86e64bc6122964f006837", pinkYellowGreen:"8e0152c0267edd72adf0b3d6faddedf5f3efe1f2cab6de8780bb474f9125276419", 
    ...
    darkGreen:"3a3a3a215748006f4d048942489e4276b340a6c63dd2d836ffeb2cffffaa", darkMulti:"3737371f5287197d8c29a86995ce3fffe800ffffff", darkRed:"3434347036339e3c38cc4037e75d1eec8620eeab29f0ce32ffeb2c"}, _ => interpolateColors(colors(_)));
    const defaultFormatter = value => vegaUtil.isArray(value) ? value.map(v => String(v)) : String(value), ascending = (a, b) => a[1] - b[1], descending = (a, b) => b[1] - a[1], symbols = {quantile:"quantiles", quantize:"thresholds", threshold:"domain"}, formats = {quantile:"quantiles", quantize:"domain"}, formatRange = format => (value, index, array) => {
      index = array[index + 1];
      array = array.max;
      array = array != null ? array : Infinity;
      index = index != null ? index : array;
      value = Number.isFinite(value) ? format(value) : null;
      index = Number.isFinite(index) ? format(index) : null;
      return value && index ? value + " – " + index : index ? "\x3c " + index : "≥ " + value;
    }, formatDiscrete = format => (value, index) => index ? format(value) : null, formatPoint = format => value => format(value);
    exports.Band = "band";
    exports.BinOrdinal = "bin-ordinal";
    exports.DiscreteLegend = "discrete";
    exports.Diverging = "diverging";
    exports.GradientLegend = "gradient";
    ...
    exports.Sqrt = "sqrt";
    exports.SymbolLegend = "symbol";
    exports.Symlog = "symlog";
    exports.Threshold = "threshold";
    exports.Time = "time";
    exports.UTC = "utc";
    exports.bandSpace = bandSpace;
    exports.domainCaption = function(locale, scale, opt) {
      opt = opt || {};
      var max = Math.max(3, opt.maxlen || 7), specifier = opt.format;
      const type = (opt = opt.formatType) || scale.type;
      vegaUtil.isString(specifier) && isTemporal(type) && (specifier = specifier.replace(/%a/g, "%A").replace(/%b/g, "%B"));
      locale = specifier || type !== "time" ? specifier || type !== "utc" ? labelFormat(locale, scale, 5, null, specifier, opt, !0) : locale.utcFormat("%A, %d %B %Y, %X UTC") : locale.timeFormat("%A, %d %B %Y, %X");
      if (isDiscretizing(scale.type)) {
        return max = labelValues(scale).slice(1).map(locale), scale = max.length, `${scale} boundar${scale === 1 ? "y" : "ies"}: ${max.join(", ")}`;
      }
      if (isDiscrete(scale.type)) {
        return specifier = scale.domain(), scale = specifier.length, max = scale > max ? specifier.slice(0, max - 2).map(locale).join(", ") + ", ending with " + specifier.slice(-1).map(locale) : specifier.map(locale).join(", "), `${scale} value${scale === 1 ? "" : "s"}: ${max}`;
      }
      max = scale.domain();
      return `values from ${locale(max[0])} to ${locale(vegaUtil.peek(max))}`;
    };
    exports.interpolate = interpolate;
    exports.interpolateColors = interpolateColors;
    exports.interpolateRange = function(interpolator, range) {
      const start = range[0], span = vegaUtil.peek(range) - start;
      return function(i) {
        return interpolator(start + i * span);
      };
    };
    exports.isContinuous = function(key) {
      return hasType(key, "continuous");
    };
    exports.isDiscrete = isDiscrete;
    exports.isDiscretizing = isDiscretizing;
    exports.isInterpolating = function(key) {
      return hasType(key, "interpolating");
    };
    exports.isLogarithmic = isLogarithmic;
    exports.isQuantile = function(key) {
      return hasType(key, "quantile");
    };
    exports.isRegisteredScale = function(scale) {
      return scale && scale[VEGA_SCALE] === !0;
    };
    exports.isTemporal = isTemporal;
    exports.isValidScaleType = isValidScaleType;
    exports.labelFormat = labelFormat;
    exports.labelFraction = function(scale) {
      var domain = scale.domain();
      const count = domain.length - 1;
      let lo = +domain[0];
      domain = +vegaUtil.peek(domain);
      let span = domain - lo;
      scale.type === "threshold" && (scale = count ? span / count : 0.1, lo -= scale, domain += scale, span = domain - lo);
      return value => (value - lo) / span;
    };
    exports.labelValues = labelValues;
    exports.quantizeInterpolator = function(interpolator, count) {
      const samples = Array(count), n = count + 1;
      for (let i = 0; i < count;) {
    ...
    exports.scale = scale;
    exports.scaleCopy = function(scale) {
      const t = scale.type;
      scale = scale.copy();
      scale.type = t;
      return scale;
    };
    exports.scaleFraction = function(scale$1, min, max) {
      var delta = max - min;
      let t, s;
      return delta && Number.isFinite(delta) ? (delta = (t = scale$1.type).indexOf("-"), t = delta < 0 ? t : t.slice(delta + 1), s = scale(t)().domain([min, max]).range([0, 1]), scaleProps.forEach(m => scale$1[m] ? s[m](scale$1[m]()) : 0), s) : vegaUtil.constant(0.5);
    };
    exports.scaleImplicit = implicit;
    exports.scheme = scheme;
    exports.tickCount = function(scale, count, minStep) {
      let step;
      vegaUtil.isNumber(count) && (scale.bins && (count = Math.max(count, scale.bins.length)), minStep != null && (count = Math.min(count, Math.floor(vegaUtil.span(scale.domain()) / minStep || 1) + 1)));
      vegaUtil.isObject(count) && (step = count.step, count = count.interval);
      vegaUtil.isString(count) && (count = scale.type === "time" ? vegaTime.timeInterval(count) : scale.type == "utc" ? vegaTime.utcInterval(count) : vegaUtil.error("Only time and utc scales accept interval strings."), step && (count = count.every(step)));
      return count;
    };
